...
func (c *Collector) GetCPUPercent() (float64, error) {
	...
	var total float64
	for _, p := range percent { total += p }
	if len(percent) == 0 { return 0, nil }
...
func init() {
	...
	if err == nil && len(counters) > 0 { lastNetIO = &counters[0] }
	lastNetTime = time.Now()
}

func (c *Collector) GetNetworkRate() (float64, float64, error) {
	counters, err := net.IOCounters(false)
	if err != nil || len(counters) == 0 { return 0, 0, err }
	now := time.Now(); curr := counters[0]
	elapsed := now.Sub(lastNetTime).Seconds()
	if elapsed <= 0 || lastNetIO == nil { lastNetIO = &curr; lastNetTime = now; return 0, 0, nil }
	bytesRecv := float64(curr.BytesRecv - lastNetIO.BytesRecv)
	bytesSent := float64(curr.BytesSent - lastNetIO.BytesSent)
	dl := bytesRecv/elapsed/1024; ul := bytesSent/elapsed/1024
	lastNetIO = &curr; lastNetTime = now
	return dl, ul, nil
}
...
func (c *Collector) GetTopProcesses(n int) ([]entity.ProcessInfo, []entity.ProcessInfo, error) {
	...
	if err != nil { return nil, nil, fmt.Errorf("无法获取 PID 列表: %w", err) }
	...
	for _, p := range processesList {
		if p == nil { continue }
		cpuPercent, err := p.CPUPercent(); if err != nil { continue }
		memInfo, err := p.MemoryInfo(); if err != nil { continue }
		memPercent, err := p.MemoryPercent(); if err != nil { continue }
		name, err := p.Name(); if err != nil { name = "unknown" }
		info := entity.ProcessInfo{ PID: p.Pid, Name: name, CPUPercent: cpuPercent, MemPercent: memPercent, MemRSS: memInfo.RSS/1024/1024 }
		if cpuPercent > 0.1 { cpuList = append(cpuList, info) }
		if memPercent > 0.1 { memList = append(memList, info) }
	}
	...
	if len(cpuList) > n { cpuList = cpuList[:n] }
	if len(memList) > n { memList = memList[:n] }
