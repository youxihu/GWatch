...
type MySQLMetrics struct {
	// 连接与会话指标
	Connections     ConnectionMetrics     `yaml:"connections"`
	
	// 查询性能指标
	QueryPerformance QueryPerformanceMetrics `yaml:"query_performance"`
	
	// InnoDB Buffer Pool指标
	BufferPool    BufferPoolMetrics    `yaml:"buffer_pool"`
	
	// 复制状态指标（主从架构）
	Replication   *ReplicationMetrics  `yaml:"replication,omitempty"`
	
	// 锁与阻塞指标
	Locks         LockMetrics          `yaml:"locks"`
	
	// 事务与日志指标
	Transactions  TransactionMetrics   `yaml:"transactions"`
...
type ScheduledPushMetrics struct {
	...
	TickerMetrics    *TickerMetrics   `json:"ticker_metrics,omitempty"`
}
