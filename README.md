...
│   ├── config_client2.yml        # Client模式测试配置2
│   ├── config_server.yml         # Server模式测试配置
│   └── config_example.yml        # 配置示例
│
├── scripts/                       # 测试脚本目录
│   ├── README.md                 # 脚本使用说明
│   ├── start_all.sh              # 快速启动三个实例
│   ├── test_client_server.sh     # 多客户端聚合测试
│   ├── test_high_io.sh           # 高IO负载测试
│   └── cleanup.sh                # 清理测试文件脚本
│
└── internal/                      # 内部代码
...
├── logs/                          # 日志目录（自动生成，已忽略）
│   ├── gwatch.log                # 运行日志
│   └── scheduled_push/           # 定时推送数据日志
│       ├── client/                # 客户端数据日志（JSON）
│       └── server/                # 服务器报告日志（Markdown）
│
├── bin/                           # 编译输出目录
│   └── Gwatch                    # 可执行文件
│
...
项目提供了多个测试脚本，位于 `scripts/` 目录下：

- **`start_all.sh`**: 快速启动三个 GWatch 实例进行测试
- **`test_client_server.sh`**: 测试多客户端聚合功能（启动一个Server和两个Client）
- **`test_high_io.sh`**: 模拟高网络IO和高磁盘IO，测试监控系统在高负载下的表现
- **`cleanup.sh`**: 清理测试产生的临时文件和日志
